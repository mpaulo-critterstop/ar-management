...
export default function DispatchPage() {
  ...
  const runSync = async () => {
    setSyncing(true);
    try {
      const res = await fetch('/api/dispatch/sync', {
        method: 'POST',
  ...
  const getFlags = (job: any) => {
    ...
    if (job.hasTrapping && job.trapCheckCount >= 3 && !job.closedOut) flags.push(`${job.trapCheckCount} trap checks`);
    return flags;
  };
  ...
  return (
    <div style={{ padding: '0 24px 24px', maxWidth: 1300, margin: '0 auto' }}>
      {toast && <div style={{ position: 'fixed', top: 16, right: 16, zIndex: 2000, background: '#E1F5EE', color: '#085041', border: '0.5px solid #5DCAA5', borderRadius: 8, padding: '10px 16px', fontSize: 13, fontWeight: 500 }}>{toast}</div>}
      ...
      {kpis && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            ...
            { label: 'Closed this month', value: kpis.closedThisMonth, filter: 'all', color: '#1D9E75' },
          ].map(tile => (
            <div
              key={tile.label}
