        ...
        try {
          ...
          if (existing) {
            await prisma.customer.update({
              ...
              data: { name, email: fc.email||undefined, phone: fc.phone||undefined, status }
            });
            customersUpdated++;
          } else {
            await prisma.customer.create({
              data: { name, email: fc.email||undefined, phone: fc.phone||undefined, status, externalId: String(fc.customerID), externalSource: "fieldroutes", terms: "Net 30" }
            });
            customersCreated++;
          }
        ...
        try {
          ...
          if (existing) {
            ...
            await prisma.invoice.create({
              data: { id: invId, customerId: customer.id, date: new Date(fi.serviceDate), due: new Date(fi.dueDate), amount: fi.total, paid, status, externalId: String(fi.invoiceID), externalSource: "fieldroutes", serviceType: "FieldRoutes" }
            });
            invoicesCreated++;
          }
