...
export async function GET(req: NextRequest) {
  ...
  const log: string[] = [
    `Office: ${officeFilter}`,
    `thirtyDayB range: ${rangeStartStr} → ${rangeEndStr} (days 16-30)`,
  ];
  const rl = makeRateLimiter();

  try {
    // ── Read thirtyDayA cache ──
    const cacheKey = `fp_30da_${officeFilter}_${weekEndStr}`;
    const cached = await prisma.appSetting.findUnique({ where: { key: cacheKey } });
    if (!cached) {
      return NextResponse.json({
        status: 'error',
        error:  `No thirtyDayA cache found for ${officeFilter} ${weekEndStr}. Run /thirtyDayA first.`,
      }, { status: 400 });
    }
    ...
    // ── Process routes ──
    const bCounts = new Map<string, { completed: number; pending: number; noShow: number }>();
    ...
    for (const route of routes) {
      ...
      const prev = bCounts.get(tech) || { completed: 0, pending: 0, noShow: 0 };
      bCounts.set(tech, {
    ...
    for (const techID of allTechIDs) {
      ...
      const a = aCounts[techID]      || { completed: 0, pending: 0, noShow: 0 };
      const b = bCounts.get(techID)  || { completed: 0, pending: 0, noShow: 0 };
