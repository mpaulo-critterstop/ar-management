...
async function syncOffice(office: string, horizonDays: number) {
  ...
  for (let i = 0; i < subIds.length; i += 1000) {
    ...
    for (const s of (got?.subscriptions || [])) {
      ...
      // Require a real last-completed service. A recurring sub with NO completed service isn't "due for its
      // next service" — it's brand-new (and would be scheduled) or an abandoned/deleted account (FR returns
      // an active customer but the subscription itself is gone). This is what surfaced ghost accounts.
      if (!lastRaw) { neverServicedSkipped++; continue; }
