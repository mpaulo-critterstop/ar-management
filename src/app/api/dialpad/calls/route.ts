  ...
  const [statsRows, agentRows, trackingRows, dailyRows, firstTimeRows] = await Promise.all([
    ...
    prisma.$queryRaw<Array<any>>`
      SELECT
        ...
        COUNT(*) FILTER (WHERE direction = 'inbound' AND state != 'answered' AND target_name != '') as agent_missed,
        COUNT(*) FILTER (WHERE direction = 'inbound' AND state != 'answered' AND target_name = '') as missed_opportunity,
